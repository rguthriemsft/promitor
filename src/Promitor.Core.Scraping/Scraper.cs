        ...
        where TMetricDefinition : MetricDefinition, new()
    {
        ...
        public async Task ScrapeAsync(MetricDefinition metricDefinition)
        {
            try
            {
                ...
                var labels = new string[1] { "source" };
                var gauge = Metrics.CreateGauge(metricDefinition.Name, metricDefinition.Description, includeTimestamp: metricsTimestampFeatureFlag, labelNames: labels);
                gauge.WithLabels("azure_monitor").Set(foundMetricValue);
            }
